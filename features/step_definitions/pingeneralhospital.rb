...
And ("there are general hospitals near me") do
  expect(page).to have_xpath "//script[contains(., icon.pin_hospitalgeral)]", visible: false
end
...
And ("there are not general hospitals near me") do
  expect(page).not_to have_xpath "//script[contains(., icon.pin_hospitalgeral)]", visible: false
end
